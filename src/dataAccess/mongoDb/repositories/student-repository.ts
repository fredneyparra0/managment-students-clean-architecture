...
export class RepositoryStudentMongoDb {
  private readonly modelStudent: ModelStudentMongoDb = new ModelStudentMongoDb()

  async findAllStudents (): Promise<IStudent[]> {
    try {
      return await this.modelStudent.modelStudentMongo.find({ isDeleted: false }).select(['-password', '-isDeleted'])
    } catch (err: unknown) {
  ...
  async updateStudent (idStudent: string | number | ObjectId, data: IStudent): Promise<IStudent | null> {
    try {
      ...
      return await this.modelStudent.modelStudentMongo.findByIdAndUpdate(idStudent, data)
  ...
  private convertIdFromString (id: number): string {
    return id.toString()
  }
}
